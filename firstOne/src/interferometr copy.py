...
class Interferometer:
    ...
    def send_command(self, command, timeout=3):
        ...
        try:
            ...
            response = self.client.recv(1024).decode().strip().split("\r\n")[0]
            self.client.settimeout(None)  # Reset timeout to None for future operations
            return response
...
signal_strength2 = round(float(interferometer.get_return_value("SignalStrength")) * 3.226)
...
if signal_strength2 is not None:
    ...
    print("Failed to get signal strength2")
if laser_measurement is not None:
    print(f"Laser measurement: {laser_measurement} m")
else:
